...
router.get("/user/test_ep", async (req, res) => {
  //   const user = await Users.findById(req.params.user_id);
  return res.json({ sts: 200, msg: "Test success, endpoint is working!" });
});
...
router.post("/user-add", upload.single("image"), async (req, res) => {
  ...
  if (findUser) {
    ...
    return res.json({
      msg: `Bunday ${req.body.first_name} ro'yxatga olingan!`,
    });
  }
  ...
  return res
    .status(201)
    .json({
      msg: "succes new User saved",
      sts: 201,
      token: JWT.sign({ _id: user._id }, process.env.JWT_PASSWORD),
      user,
    });
});
